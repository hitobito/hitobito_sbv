...
module Sbv::PaperTrail::VersionAssociationChangePresenter
  ...
  def association_change_text(_changeset)
    if event == "create" && item_type == "Role"
      role_create_association_change_text || super(nil)
    elsif event == "update"
      super(changeset_list)
    else
      super(nil)
    end
  end

  def role_create_association_change_text
    rendered = role_create_changeset_list
